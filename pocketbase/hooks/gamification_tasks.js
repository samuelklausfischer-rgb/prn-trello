      ...
      try {
        ...
        for (const ua of userAchs) {
          ...
          try {
            ...
            $app.logger().error('Error processing achievement', 'error', String(err))
          }
        }
      } catch (err) {
        $app.logger().error('Error updating user gamification', 'error', String(err))
      }
    }
  } catch (err) {
    $app.logger().error('Fatal error in gamification_tasks hook', 'error', String(err))
  }
  return e.next()
}, 'tasks')
